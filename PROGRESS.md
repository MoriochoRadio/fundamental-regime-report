...
**마지막 갱신**: 2026-05-19 (커밋 2 적용 — D2 후보 상태 되돌림 + §7.4 ruff format 규칙 박제)
...
> **다음 세션은 이 지점부터 이어간다**:
> 1. **후보 A 검증 — A1 우선** (커밋 2 완료, 2026-05-19):
>    - (A1) 한국 신용등급 변동 이력 2015-2024·유니버스 321 종목 자동
>      확보 출처 존재 여부 조사. DART 텍스트 파싱이 필요하면 B1.2급
>      (정정 본문 파싱, "1인 범위 초과"로 기각) 와 동일 함정인지 정직
>      평가 후 보고. **A1 결과를 먼저 보고 — 불확실하면 A2 진행 금지.**
>    - (A2, A1이 "확보 가능" 판정 시에만) *N단계 하향* 라벨로 양성 수·
>      비율·walk-forward 연도 분포·spot-check 산출 → **α (27건·8.4%,
>      0년 2개) 와 나란히 비교표** 제시.
>    - 판정: A 데이터 확보 + 라벨 품질 통과면 *A로 D2 확정* (의미
>      보존 우선). A 가 B1.2급 부담이면 A 기각, *B (α) 로 D2 확정*.
> 2. 단계 2 진입 조건: **CI 그린 (✅ 2026-05-19 회복)** + **D2 최종 확정**.
...
- **단계**: 단계 1 코드 작업 완료. 단계 2 진입은 **CI 그린(✅ 회복) +
  D2 최종 확정** 두 조건 모두 충족 후. 현재 D2 는 *후보 A(신용등급)
  검증 대기* 상태 (커밋 2 적용 완료, 2026-05-19).
- **요약**: CI 4회 연속 실패(2026-05-18) 의 원인은 *ruff format --check*
  단계의 미정리 7개 파일 → 커밋 1 (`71ef11a`, 2026-05-19) 로 `ruff format`
  적용 후 그린 회복. 커밋 2 (D2 revert 6항목 + CLAUDE.md §7.4 "코드 커밋
  전 ruff format 필수" 규칙 1줄) 적용 완료. 다음: D2 후보 A (신용등급)
  검증 — A1 자동 확보 출처 존재성 확인부터.
...
### 3.0. 커밋 2 적용 완료 (2026-05-19)

**상태**: ✅ 적용 완료. 사용자 승인 후 7건 모두 반영 (PROGRESS 4 + CLAUDE.md 3).
커밋 2 메시지: `docs(d2): revert D2 to candidate state + enforce ruff format pre-commit`.

**반영된 7건**:

1. ✅ PROGRESS §4 D2 행: `🟡 후보 검증 중` 로 정정 (A1·A2 단계 명시)
2. ✅ PROGRESS §1 현재 상태: "단계 1 코드 작업 완료. 단계 2 진입은 CI 그린
   + D2 최종 확정 두 조건 충족 후" 로 정정
3. ✅ PROGRESS §5.5.7 헤더: `D2 1차 후보 (α=상폐∪B1') — 사용자 요청으로
   미확정 되돌림 (2026-05-19)` + 본문 첫 주석 박스에 *후보 B 계열 확정
   진단치* 안내
4. ✅ PROGRESS §5 결정 로그: 2026-05-19 정정 한 줄 추가 ("α 후보 B 격하")
5. ✅ CLAUDE.md §3.1 D2 박제: `D2 후보 검증 단계 — 최종 확정 시 갱신`
6. ✅ CLAUDE.md §4.1 D2 단락: `D2 1차 후보 / 미확정` (수치는 후보 B 보존)
7. ✅ CLAUDE.md §7.4: ruff format 필수 규칙 한 줄 (CI 4회 실패 재발 방지)

### 3.1. 후보 A (신용등급) 검증 — 커밋 2 후 (현 진행)
...
| D2 | 부실(라벨) 정의 | **후보 A (신용등급 N단계 하향)** vs **후보 B (α=상폐∪B1', 27건·8.4%, 0년 2)** | 🟡 **후보 검증 중** — A1 자동 확보 출처 존재성 조사 → 가능 시 A2 라벨 산출·비교표 → 확정 (사용자 요청 2026-05-19). §5.5.7 |
...
### 5.5.7. D2 1차 후보 (α = 상폐 부실 ∪ B1') — 사용자 요청으로 미확정 되돌림 (2026-05-19)

> **주의 (2026-05-19 갱신)**: 본 절의 모든 수치(양성 27·8.4%·B1' 19·중복 0·
> 0년 2·spot-check)는 **후보 B (α = 상폐 부실 ∪ B1') 계열의 확정 진단치**다.
> 사용자 요청으로 후보 A (신용등급) 와 동일 진단 틀로 비교 검증 후 D2 최종
> 확정. A1 (자동 확보 출처 존재성) 결과에 따라 A 채택·기각 분기.
...
**β 학습 측 보완** (단계 2 진입 시):
- **class weight**: 양성 비율 8.4%에 비례한 weight (또는 `compute_class_weight='balanced'`)
- **forward window 2년** ablation: 양성 추가 확보 (단 분석 마지막 2년 손실)
- **bootstrap / SMOTE**: 0년(2021·2023) 보완. *시계열 누수* 회피 위해 *시점 내* 적용
