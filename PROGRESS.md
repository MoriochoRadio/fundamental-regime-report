...
- [ ] **★ CFS/OFS 혼재 데이터 일관성 + 지주회사 부실 신호 희석 점검**
  ...
  - **지주회사 CFS 부실 신호 희석 검증**: 지주회사의 CFS 는 자회사 손익이
    통합되어 *모회사 단독의 부실*이 가려질 수 있음. OFS fallback 적용 시
    *어떤 종목이 어느 보고서에서 영업이익 부호 다른지* 매핑. 가능하면
    D2 라벨 B1' (영업이익 음수 전환) 의 영향 정량화 — *지주회사 종목 분리
    시 양성 27 ± n*. 변동 보고.
  - **D2 라벨 (상폐 부실) 영향**: 상폐 부실 8건 중 지주회사 비율 확인.
    지주회사 분리 시 양성 분포 변동 보고.
  ...
  - **시점**: labels.py v1 (CFS only 캐시 재사용) 완료 후, step #3 (D10
    OFS fallback 적용) 시 검증. labels.py v1 의 27 양성 reproducibility 와
