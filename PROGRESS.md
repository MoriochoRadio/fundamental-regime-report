...
2. **D2 라벨 견고성 모델 단계 결정**: 부호 차이 9 cells 가 labels.py 의 B 신호 판정에 영향. 본 시점에 라벨 재산출은 *(§5.5.10 결정: 라벨 측 보강 안 함)*. 모델 단계에서 *fs_div 별 stratification* 또는 *fs_div as feature* 결정.
...
**남기는 이유**: features 빌더 작성 시점에 fs_div 처리 설계가 *경험적 증거* 없이 결정되지 않도록. (i) 컬럼 동행 결정 (§5.5.14) 의 정당화 + 모델 단계에서 fs_div 활용 결정의 *baseline 자료*. PROGRESS §3 DoD 의 점검 항목 해소.
