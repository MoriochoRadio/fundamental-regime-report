...
- **재구축 현황 (단위 a~l + QA)**: 컴포넌트 8종(utils·data_loader·header·
  ...
  말미 + 본 §5 최상단.
- **성능·UX 후속 (2026-06-02)**: main 병합 후 QA 진단으로 (1) 차트 성능 병목
...
### 3.★ 현재 다음 (2026-06-02) — 단계 5 마무리

- [x] **README/문서 §7.7 외부 노출 정리** — 완료 (`a036c02`).
...
- **2026-06-02** — 종목 분석 UX 보완 (`17fde0a`). QA 진단으로 **#1 위험 점수
  ...
  *정상(버그 아님)* 확인. 위에 친화 보완: 종목 선택 시 최신 평가 시점 자동
  점프 + 희소 이유 일반어 안내 + 상폐 종목 차트 거래 종료 caption(날짜만,
  사유 미노출). `load_delisting` 은 **UI caption 전용·§5 피처 격리 무관**(app
  레이어). 데이터/모델 무변경.
- **2026-06-02** — 차트 성능 수정 (`8e4092e`). PriceChartWithStateOverlay 의
  상태 배경이 block(741개)마다 `add_vrect` 호출 → 호출마다 layout.shapes 전체
  복사·재검증 = **O(n²)**, 측정 ~115초(인터랙션 지연 단독 병목). 동일 shape
  dict 리스트를 **1회 `update_layout(shapes=...)`** 로 배치 할당 = O(n) →
  figure ~0.16초(약 700배). 시각 결과 동일. *캐시·로더 아닌 figure 생성이
  병목* 임을 측정으로 규명 후 수정 (자문 진단 → 측정 → 수정 사이클).
