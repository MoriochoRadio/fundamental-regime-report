...
3. ~~단계별 진행 계획~~ ✅
4. **단계 1 진입** — 다음 작업:
   1. D1(유니버스), D7(재무 lag), 분석 기간 결정 ← **지금**
   2. `pyproject.toml` 작성 + `uv sync`
   3. `configs/data.yaml` 작성
   4. `src/frr/data/{dart, krx, calendars, cache}.py` 작성
   5. `scripts/collect_data.py` + 룩어헤드 차단 테스트 작성
   6. CI 워크플로(lint+test) 추가
