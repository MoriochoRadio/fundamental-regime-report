...
**마지막 갱신**: 2026-05-19 (§5.5.9 박제 — D2 distress 필터 결함 발견·P1 정정 경위 고정. 단계 2 진입은 *D2 정정 완료 후*로 유예)
...
> **다음 세션은 이 지점부터 이어간다**:
> 1. **D2 정정 진행 중** (2026-05-19 §5.5.9 박제 후) — 단계 2 진입 *유예*:
>    - 발견: A 8 중 7건이 합병성 자발 해산 (부실 신호 아님). §5.5.7 의
>      "양성 27·A 8" 박제가 부정확함이 *A 8 육안 게이트* 로 포착.
>    - 결정: P1 (ToSymbol/ToName 구조적 신호로 필터 보강 — 자의적 키워드 X)
>    - 진행 단계: (1) ToSymbol 신뢰성 검증 → (2) 보강 필터 + 재진단 → (3)
>      재캡처 + (2)게이트 재수행 → (4) §5.5.7·CLAUDE.md·capture 결과 정정
>    - 각 단계 사용자 확인 게이트. labels.py 코드는 정정 완료 + 재검증 통과
>      후에만 시작.
> 2. **단계 2 진입 조건 갱신**: D2 ✅ → 🟡 *정정 중*. CI ✅ + CRLF ✅ 유지.
>    단계 2 첫 작업 계획(labels.py 등) 은 D2 정정 후 재확인.

---

## 1. 현재 상태 (Current Status)

- **단계**: 단계 1 코드 작업 완료. **D2 정정 진행 중 (§5.5.9, 2026-05-19)** —
  CI ✅ + CRLF ✅ 유지하나 D2 는 🟡 *distress 필터 결함 P1 정정 중* (A 8 중
  7 합병성 해산이 §5.5.7 의 양성 27 박제에 노이즈로 포함). 단계 2 진입은
  *D2 정정 완료 + 재검증 통과 후*로 유예.
...
### 5.5.9. ★ D2 distress 필터 결함 — A 8 중 7 합병성 자발 해산 (2026-05-19)
...
**결정 — 옵션 (P1) 채택**: *키워드 땜질이 아닌* **구조적 신호 사용**

- **폐기 옵션**: `"해산"` 단독 제외 / 강한 키워드만 허용 — *또 다른 자의적
  키워드 땜질*. B1 v1→v2 정밀화에서 실패한 *키워드 정밀화로 노이즈 잡기*
  패턴의 반복 위험.
- **채택 방향**: **`ToSymbol/ToName` 컬럼의 구조적 신호** — 합병은 본질적으로
  *승계 법인 존재* (ToSymbol/ToName 채워짐), 부실 상폐는 *승계 없음*
  (비어 있음). 키워드보다 *자의성 없는 판별 기준*.
- **검증 절차** (다음 단계): ToSymbol 의 (a) 합병 7건 채워짐 (b) 포스코플랜텍
  비어 있음 (c) 전체 delisting 결측률·신뢰성 — 데이터로 실측 후 사용자 확인.
...
**예상 영향** (재진단 후 §5.5.7 정정 시 확정 수치로 갱신):
- A 양성 수: 8 → **~1** (포스코플랜텍만, ToSymbol 부재 가정)
- α 합집합 양성: 27 → **~20** (A ~1 + B 19, 중복 0 가정)
- 학습 임계(30) 에 더 빠듯해짐 — 단계 2 학습 측 보완(class weight·forward
  2년 ablation·bootstrap·시점별 가중치) 정밀 설계 *더 중요*. 양성이 줄어드는 게
  불편하다고 필터를 느슨하게 하지 않음.
- 0년 분포 변동 예상: A 합병 7건이 빠지면 2015(3 → ?)·2017(5 → ?)·2022(2 → ?)
  등에 영향. 재진단으로 확정.

**진행 절차** (사용자 명시, 각 단계 확인 게이트):
1. ToSymbol/ToName 신뢰성 검증 (합병 7 채워짐 / 포스코플랜텍 비어 / 전체
   결측률·구분력) — 데이터로 실측, 사용자 확인.
2. 보강 필터 적용 + 진단 재실행 → 새 양성 수 산출, 사용자 확인.
3. 재캡처 + (2)게이트 재수행 (연도 분포·spot-check 의미 검증, A 의 ToSymbol
   부재 포함), 사용자 확인.
4. §5.5.7 양성 수치 정정 + CLAUDE.md §3.1/§4.1 정정 + capture 결과 정정.

**남기는 이유** (§5.5.7 D2 여정·§4-pre CRLF 정정 보존과 같은 원칙):
"§5.5.5 경고 → §5.5.7 누락 → A 육안 게이트로 박제 직전 포착 → P1 정정"
의 전 경위는 *방법론적 추적성의 증거*. *누락과 포착의 기록 자체가* 사후
정직성의 가치를 보존한다. 또한 *사용자 강화 검증 안전장치 (A 육안 게이트)
가 결함을 잡은 사례* 로 향후 동일 패턴 (대량 라벨 박제 전 spot-check 의무화)
의 표준 절차 근거가 된다.
