...
### 4.5 국면 전환 빈도 높음 (choppiness)
- 분석 기간 2,273 거래일 동안 **741 구간** (상태 연속 블록) — 평균 구간
  길이 **약 3.1 거래일**. 일별 분류 + 평활화 없음 → 단기 노이즈에 민감.
- ★ 전이 행렬은 자기 지속성이 강한데 (§3.2) 실제 경로는 전환이 잦음 —
  *평균적 지속성* 과 *잦은 단기 반전* 이 공존하는 tension 을 정직 기록.
  시각 명료성과 "국면" 으로서의 해석 안정성에 한계.
- 평활화 (최소 지속 기간 규칙·과거 방향 다수결 윈도 등) 는 후속 과제 —
  단, 적용 시 *시점 t 이전 데이터만* 쓰는 과거 방향 윈도로 한정해야
  사전적(causal) 산출 원칙 (CLAUDE.md §5) 과 충돌하지 않는다.
- 대시보드 한계 페이지에는 일반어 1줄로 반영 (원본 수치 미노출).
